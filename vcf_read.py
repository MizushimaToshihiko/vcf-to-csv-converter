...
def vcf_write_to_csv(vcf, vcf_path, vcf_enc) -> None:
    ...
    with open(csv_filename, mode="w", encoding="shift_jis", errors="ignore") as f2:
        ...
        title = f"VERSION,X-DCM-EXPORT,X-DCM-ACCOUNT;DOCOMO,N,N;CHARSET={VCF_ENC},SOUND;X-IRMC-N;CHARSET={VCF_ENC},X-DCM-SOUND-ORGINAL;X-IRMC-N;CHARSET={VCF_ENC},TEL;CELL,X-DCM-TEL-ORIGINAL;CELL,E,TEL;PREF;WORK,TEL;PREF;CELL,X-GNO,X-GN;CHARSET={VCF_ENC},X-DCM-GN-ORIGINAL;CHARSET={VCF_ENC},X-DCM-GROUP-ICONCOLOR,X-DCM-GROUP-ICON,EMAIL;CELL,X-DCM-EMAIL-ORIGINAL;CELL,X-DCM-RINGTONE,NOTE;CHARSET={VCF_ENC},TEL;WORK,X-DCM-TEL-ORIGINAL;WORK,ADR;CHARSET={VCF_ENC},X-DCM-POSTALCODE-ORIGINAL,TEL;CUSTOM,X-DCM-LABEL;CHARSET={VCF_ENC},X-DCM-TEL-ORIGINAL;CUSTOM,TEL;HOME,X-DCM-TEL-ORIGINAL;HOME,NICKNAME;DEFAULT;CHARSET={VCF_ENC},TEL;VOICE,X-DCM-TEL-ORIGINAL;VOICE,NOTE;ENCODING=QUOTED-PRINTABLE;CHARSET={VCF_ENC}" 
        title = title.split(",")
        print("title:", title)
        f2.write(",".join(title) + "\n") # 列名を書き込み
