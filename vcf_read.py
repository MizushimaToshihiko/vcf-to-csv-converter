...
def vcf_write_to_csv(vcf, vcf_path, vcf_enc) -> None:
    ...
    with open(csv_filename, mode="w", encoding="shift_jis", errors="ignore") as f2:
        ...
        title = f"N,N;CHARSET={VCF_ENC},TEL;CELL,E,TEL;PREF;WORK,TEL;PREF;CELL,TEL;WORK,TEL;CUSTOM,EMAIL;CELL,EMAIL;PREF;CELL,TEL;VOICE,TEL;HOME,VERSION,X-DCM-EXPORT,X-DCM-ACCOUNT;DOCOMO,X-DCM-TEL-ORIGINAL;CELL,X-DCM-EMAIL-ORIGINAL;CELL,X-DCM-RINGTONE,NOTE;CHARSET={VCF_ENC},X-DCM-TEL-ORIGINAL;WORK,ADR;CHARSET={VCF_ENC},X-DCM-POSTALCODE-ORIGINAL,SOUND;X-IRMC-N;CHARSET={VCF_ENC},X-DCM-SOUND-ORGINAL;X-IRMC-N;CHARSET={VCF_ENC},X-GNO,X-GN;CHARSET={VCF_ENC},X-DCM-GN-ORIGINAL;CHARSET={VCF_ENC},X-DCM-LABEL;CHARSET={VCF_ENC},X-DCM-TEL-ORIGINAL;CUSTOM,X-DCM-GROUP-ICONCOLOR,X-DCM-GROUP-ICON,X-DCM-TEL-ORIGINAL;HOME,NICKNAME;DEFAULT;CHARSET={VCF_ENC},X-DCM-TEL-ORIGINAL;VOICE,NOTE;ENCODING=QUOTED-PRINTABLE;CHARSET={VCF_ENC}" 
        title = title.split(",")
        print("title:", title)
        f2.write(",".join(title) + "\n") # 列名を書き込み
