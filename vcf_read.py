...
def vcf_write_to_csv(vcf, vcf_path, vcf_enc) -> None:
    ...
    with open(csv_filename, mode="w", encoding="shift_jis", errors="ignore") as f2:
        # f2.write(vcf)
        title = "VERSION,X-DCM-EXPORT,X-DCM-ACCOUNT;DOCOMO,N;CHARSET=%s,SOUND;X-IRMC-N;CHARSET=%s,X-DCM-SOUND-ORGINAL;X-IRMC-N;CHARSET=%s,TEL;CELL,X-DCM-TEL-ORIGINAL;CELL,E,X-GNO,X-GN;CHARSET=%s,X-DCM-GN-ORIGINAL;CHARSET=%s,X-DCM-GROUP-ICONCOLOR,X-DCM-GROUP-ICON,EMAIL;CELL,X-DCM-EMAIL-ORIGINAL;CELL,X-DCM-RINGTONE,NOTE;CHARSET=%s,TEL;WORK,X-DCM-TEL-ORIGINAL;WORK,ADR;CHARSET=%s,X-DCM-POSTALCODE-ORIGINAL,TEL;CUSTOM,X-DCM-LABEL;CHARSET=%s,X-DCM-TEL-ORIGINAL;CUSTOM,TEL;HOME,X-DCM-TEL-ORIGINAL;HOME,NICKNAME;DEFAULT;CHARSET=%s,TEL;VOICE,X-DCM-TEL-ORIGINAL;VOICE,NOTE;ENCODING=QUOTED-PRINTABLE;CHARSET=%s" % (vcf_enc.upper())
        title = title.split(",")
        print("title:", title)
        f2.write(",".join(title) + "\n") # 列名を書き込み
